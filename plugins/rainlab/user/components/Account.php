...
class Account extends ComponentBase
{
        ...
        try {
            ...
            */
            ...
            if ($redirect = $this->makeRedirection(true)) {
                //save session bookmarks after logging in
                $saved_favourites = Session::get('guestFavourites', function() { return 'Not found'; });
                if ($saved_favourites != 'Not found') {
                $user = Auth::getUser();
                foreach ($saved_favourites as $key => $value) {
                  $isBookmarked = Favourite::where('user_id', '=' , $user->id)
                                              ->where('product_id', '=' , $value)->count();
                  if (!$isBookmarked) {
                    $favourites = new Favourite;
                    $favourites->user_id = $user->id;
                    $favourites->product_id = $value;
                    $favourites->save();
                  }
                }
    ...
    public function onRegister()
    {
        try {
            ...
            $registered_group = UserGroup::where('code', 'registered')->first();
            DB::table('users_groups')->insert([
                ['user_id' => $user->id, 'user_group_id' => $registered_group->id]
            ]);
            ...
            if ($automaticActivation || !$requireActivation) {
                Auth::login($user);
                //save session bookmarks after registration or email confirmation
                $saved_favourites = Session::get('guestFavourites', function() { return 'Not found'; });
                if ($saved_favourites != 'Not found') {
                  foreach ($saved_favourites as $key => $value) {
                    $user = Auth::getUser();
