...
class Log extends Controller
{
    ...
    public function onCheckUpdate() {
    	...
    	} elseif(!empty(Input::get('txtproperty'))){
    		 
    		 $searchid = Input::get('txtproperty');

    		 $pre_qry = "https://test.ean.com/2.4/properties/content?language=en-US";

    		 $properties = Db::select('select property_id, name, dates_updated from expedia_expediaimport_properties where property_id =' . $searchid);
    		 $prop_count = 1;
        ...
        if(!empty($properties)) {
		                    ...
		                    $newresult = $value; 
		                     ...
		                     if (isset($newresult->property_id)) {
			                   ...
			                   // 	\Log::info($eps_property_id."-"." No updates");
			                    	...
			                    	//$img_count = Db::select("select COUNT(property_id) as ctr from expedia_expediaimport_images where property_id = $eps_property_id AND updated = '1'"); 
			                    	//$img_count = Db::table('expedia_expediaimport_images')->where('property_id', $eps_property_id)->where('updated','=','1')->where('links_large','<>','')->count();
			                    	$img_count = Db::table('expedia_expediaimport_images')->where('property_id', $eps_property_id)->count();
			                    	...
			                    	if($eps_img_count != $img_count) {
					                    ...
					                    // search links_large from DB to eps in case selected images are deleted
					                    $links_large_db = Db::table('expedia_expediaimport_images')->where('property_id', $eps_property_id)->where('poster','=','1')->get();
					                    $removed_links = 0;
					                    foreach($links_large_db as $link_large){
					                    	$dbImage = $link_large->links_large;

					                    	if(in_array($dbImage,$links_large_list)) {

					                    		Db::table('expedia_expediaimport_images')->where('property_id', $eps_property_id)->where('links_large','=', $dbImage)->update(['images' => 'exist']);
					                    	} else {

					                    		Db::table('expedia_expediaimport_images')->where('property_id', $eps_property_id)->where('links_large','=', $dbImage)->update(['images' => 'removed']);
		                            ...
		                            if(strtolower($change_type) == "image") {
                                        ...
                                        if($eps_img_count > $img_count) {
                                            ...
                                            $img_added = $eps_img_count - $img_count;
                                            $msg = "($img_added) new image link(s) has been added";
                                            //$msg = "EPS Images count:".$eps_img_count."/". "DB Images Count:". $img_count;

                                        } elseif($eps_img_count < $img_count) {

                                            $img_removed =  $img_count - $eps_img_count;

                                            if($removed_links > 0) {

                                                if($img_removed > 0) {

                                                    $msg = "($img_removed) image link(s) including ($removed_links) SAVED IMAGE link(s) has been removed ";
                                                } else {
                                                    $msg = "($removed_links) SAVED IMAGE link(s) has been removed ";
                                                }

                                            } else {

                                                $msg = "($img_removed) image link(s) has been removed";
                                            }
        	...
        	//\Flash::info("Properties updated - $searchid");
        ...
        } // End DB connection (!empty properties)
