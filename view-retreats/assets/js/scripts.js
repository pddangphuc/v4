...
*/
  // end auto date
  // override the rainlab user plugin error messages
    $("#popover_template_search").hide();
    $('#popover_template').hide();
    $(window).on('ajaxErrorMessage', function (event, message){

        if (message.indexOf("hashed credential") != -1) {
          alert('Incorrect username or password. Please double-check and try again.');
          // This will stop the default alert() message
          event.preventDefault();
        }

        if (message.indexOf("suspended") != -1) {
          alert("Your account has been temporarily suspended. Please try again later.");
          // This will stop the default alert() message
          event.preventDefault();
        }

        if (message.indexOf("throttled") != -1) {
          alert("You have reached the maximum number of registrations. Please try again later.");
          // This will stop the default alert() message
          event.preventDefault();
        }
    ...
    var base_url = window.location.href.split('?')[0];//get url without parameter
    var reset_code = urlParams.get('reset'); //extract the reset parameter - this will return NULL if reset isn't a parameter
      //window.location.origin + "/user/forgot-password" url of forgot password static page
    if(reset_code) { //check if reset parameter is set to anything
      if(base_url != window.location.origin + "/user/forgot-password" || base_url != window.location.origin + "/user/forgot-password/" ){ //check if page is the static page forgot password, if not show the forgot password modal
        $('#forgot-password-modal').modal('show')
        $('#popup-partialUserResetForm p').text("");
        if (performance.navigation.type == performance.navigation.TYPE_RELOAD) {// after reload user will redirected to the page with no parameter
  ...
  // search block   
    ...
    $('#id-signup-popup-forgot-password').on('click', function(e) {
        $('#login-modal').modal('hide');
        $('#login-modal').hide();
    });

    $('#id-bookings-popup-forgot-password').on('click', function(e) {
        $('#forgot-modal').modal('hide');
        $('#forgot-modal').hide();
    });

    $('#id-bookings-popup-forgot-itenerary-number').on('click', function(e) {
        $('#forgot-modal').modal('hide');
        $('#forgot-modal').hide();
    });
    ...
    $(window).scroll(function(event){
      ...
      if ( base_url == window.location.origin + "/search/" || base_url == window.location.origin + "/search" ) {
        //if the page is the search page, use this code for datepicker calendar
        if (st > lastScrollTop){
         // console.log(st + "up detected");
          if(st > 10) {
            $('#search_block').hide();
            $('.fixed').css('top', '56px');
          ...
          //console.log(st + "down detected");

        }
      }else{
        //if the page is not the search page, use this code for datepicker calendar
        if (st > lastScrollTop){
         // console.log(st + "up detected");
          if(st > 10) {
            $('#search_block').hide();
    ...
    //unwrap the a tag on hero block image on landing page and and then wrap the p tag with that a tag
    if ($("#landing_page_slider a.slide").length){ 
      var landing_slider_href = $('#landing_page_slider a.slide').attr('href'); 
      $( "#landing_page_slider .slide-image" ).unwrap();
      $( "#landing_page_slider .slide-image" ).wrap( "<div class='slide' style='width: 100%; display: inline-block;'></div>" );
      $( "#landing_page_slider p" ).wrap( "<a href='"+ landing_slider_href +"'></a>" );
    }
    ...
    //popup retrieve bookign login
     $("#form-forgot-email #forgot-button").click(function(){
          var email = $('#form-forgot-email input[name="login"]').val();
          if(ValidateEmail(email)){
            $("#form-forgot-email").hide();
            $("#form-forgot-password").show();
            $("#back-forgot").show();
            $('.popup-booking-email-show-login').text(email);
          }else{
              alert('Please Input Valid Email Address');
          }
    });

    $('.popup-booking-form-login').on('submit', function(e){
      var email = $('#form-forgot-email input[name="login"]').val();
      var password = $('#form-forgot-password #popup-booking-password').val();
      if(email === '' && !ValidateEmail(email)){
         console.log('no value');
         alert('Please Input Valid Email Address');
         return false;

      }else if(ValidateEmail(email) && password === '' && $("#form-forgot-email").is(":visible")){
            $("#form-forgot-email").hide();
            $("#form-forgot-password").show();
            $('.popup-booking-email-show-login').text(email);
            return false;
    ...
    //retrieve booking page form login
    $("#page-form-forgot-email #page-forgot-button").click(function(){
      var email = $('#page-form-forgot-email input[name="login"]').val();
      if(ValidateEmail(email)){
        $("#page-form-forgot-email").hide();
        $("#page-form-forgot-password").show();
        $('.page-booking-email-show-login').text(email);
      }else{
          alert('Please Input Valid Email Address');
      }
    });

    $('.page-booking-form-login').on('submit', function(e){
        var email = $('#page-form-forgot-email input[name="page_login"]').val();
        var password = $('#page-form-forgot-password #page-booking-password').val();
        if(email === '' && !ValidateEmail(email)){
           console.log('no value');
           alert('Please Input Valid Email Address');
           return false;

        }else if(ValidateEmail(email) && password === '' && $("#page-form-forgot-email").is(":visible")){
              $("#page-form-forgot-email").hide();
              $("#page-form-forgot-password").show();
              $('.page-booking-email-show-login').text(email);
              return false;
    ...
    $("#page-login-button").click(function(){
          ...
          var email = $('#page-form-login-email input[name="login"]').val();
          if(ValidateEmail(email)){
              $("#page-form-login-email").hide();
              $("#page-form-login-password").show();
              $("#back-login").show();
              $('.page-email-show-login').text(email);
          }else{
              alert('Please Input Valid Email Address');
          }
     ...
     $('.page-form-login').on('submit', function(e){
        var email = $('#page-form-login-password input[name="page_login"]').val();
        var password = $('#page-loginmodal-by-password').val();
        if(email === '' && !ValidateEmail(email)){
           console.log('no value');
           alert('Please Input Valid Email Address');
           return false;

        }else if(ValidateEmail(email) && password === '' && $("#page-form-login-email").is(":visible")){
              $('.page-email-show-login').text(email);
              $("#page-form-login-email").hide();
              $("#page-form-login-password").show();
              $("#back-login").show();
    ...
    $("#page-signup-button").click(function(){
          var email = $('#page-signup-by-email').val();
          if(ValidateEmail(email)){
              $("#page-form-signup-email").hide();
              $("#page-form-signup-password").show();
              $("#back-signup").show();
              $('.page-email-show-register').text(email);
          }else{
              alert('Please Input Valid Email Address');
          }
    ...
    $('.page-onRegister-form').on('submit', function(e){
        var email = $('#page-signup-by-email').val();
        var password = $('#page-form-signup-password .signup-password').val();
        var confirm_password = $('#page-form-signup-password input[name="confirm_password"]').val();
        if(email === '' && !ValidateEmail(email)){
            
           alert('Please Input Valid Email Address');
           return false;

        }else if(ValidateEmail(email) && password === '' && $("#page-form-signup-email").is(":visible")){
            
              $("#page-form-signup-email").hide();
              $("#page-form-signup-password").show();
              $("#back-signup").show();
              $('#page-form-signup-password .email-show-register').text(email);
              
              return false;
        }else if(ValidateEmail(email) && $("#page-form-signup-password").is(":visible")){
            
              if($('#page-form-signup-password .first_name').val() == ''){
                  
                  alert('First Name must not be empty');
                  return false;
                  
              }else if($('#page-form-signup-password .last_name').val() == ''){
                  
                  alert('Last Name must not be empty');
                  return false;
                  
              }else if($('#page-form-signup-password .signup-password').val() == ''){
    ...
    $('.onUpdateProfile-form').on('submit', function(e){
        ...
        var phone_code = $('input[name="iu_tel_code"]').val();
        var phone_number = $('input[name="iu_telephone"]').val();
        $('input[name="iu_telephone_full"]').val(phone_code + phone_number);
    });
...
function ClickAddRoom(){
 ...
 // var num = +$('#txtroom').val() + 1;
  ...
  var base_url = window.location.href.split('?')[0];//get url without parameter
   if ( base_url == window.location.origin + "/search" || base_url == window.location.origin + "/search/" ) {
    if (roomNr >= 3) {
      $('#popover_template').css('height', '495px');
      $('#popover_template').css('overflow-y', 'scroll');
      $('#popover_template').addClass('guest-scroll-bar');
    }
   }
}
$('.remove').on('click',function () {
    ...
    if ( base_url == window.location.origin + "/search" || base_url == window.location.origin + "/search/" ) {
      if (roomNr < 3) {
        $('#popover_template').css('height', 'auto');
        $('#popover_template').css('overflow-y', 'hidden');
        $('#popover_template').removeClass('guest-scroll-bar');
      }
   }
...
//get value location search hero block
  ...
  $("#btn-nav-location, #btn-nav-amenities").on('click', function(event) {
